...
void err_message() {
  ...
  fprintf(stdout, "\t-sh fichier_shadow a utiliser\n");
...
int open_files()
{
  ...
  if (!pass_file_ptr) {
    fprintf(stdout,
            "Impossible d'ouvrir le fichier shadow%s\n",
            pass_file);
    return(0);
  }
...
void run_exhaustif(int niveau)
{
  ...
  if (niveau == 0) {
    ...
    */
    test_encrypted_passwd = (char *) crypt(recherche, salt/* Utiliser les bons parametres */);
    if (!strcmp(test_encrypted_passwd, encrypted_passwd)) {
      fprintf (stdout, 
              "----Usager: %s, Mot de passe: \"%s\"\n", 
              user_name, 
